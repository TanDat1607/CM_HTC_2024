...
namespace CM_Local.AdsTwincat
{
    public class FB_ADS1
    {
        ...
        public object AdsRead(AdsClient TcAdsClient, string Symbol, Type SymbolType)
        {
            ...
            if (AdsCheckConnection())
            {
                try
                {
                    ...
                    {
                        result = TcAdsClient.ReadValue(Symbol, SymbolType);
                    }
        ...
        public void AdsWriteAnyString(AdsClient TcAdsClient, string Symbol, string value)
        {
            uint hValue;
            if (AdsCheckConnection())
            {
                try
                {
                    hValue = TcAdsClient.CreateVariableHandle(Symbol);
                    // other symbol types
                    TcAdsClient.WriteAnyString(hValue, value, 80, Encoding.Unicode);
                    TcAdsClient.DeleteVariableHandle(hValue);
                }
                catch (Exception err)
                {
